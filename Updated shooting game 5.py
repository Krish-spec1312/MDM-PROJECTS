...
while running:
    ...
    # Spawn UFO enemies (SLOWER)
    if random.randint(1,80) == 1 and not game_over and not game_won:
    ...
    # Move enemies (zig-zag)
    for enemy in enemies:
        enemy[1] += enemy_speed
        enemy[0] += random.randint(-2,2)

        # GAME OVER if enemy touches spaceship
        if abs(enemy[0] - player_x) < 40 and abs(enemy[1] - player_y) < 40:
            game_over = True
